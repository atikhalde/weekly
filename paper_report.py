...
HEADERS = [
    ...
    ("exit_note", "Note", 18),
    ("bars_held", "Bars", 7),
    ("gross_pnl", "Gross (Rs)", 12), ("costs", "Costs (Rs)", 11),
    ("pnl", "P&L (Rs)", 12),
...
def build_workbook(trades: list[PaperTrade], capital: float, path: Path,
                   window: str, scope_note: str = "") -> Path:
    ...
    rows = [
        ...
        ("", ""),
        ("Gross P&L (Rs)", round(sum(t.gross_pnl for t in trades), 2)),
        ("Costs (Rs)", round(-sum(t.costs for t in trades), 2)),
...
def main() -> int:
    ...
    scope = ("Scope: ALL signals (cross + deferred)" if args.include_deferred
             else "Scope: CROSS entries only - deferred fills excluded "
                  "(alerts still send both)")
    build_workbook(trades, args.capital, out, window, scope)

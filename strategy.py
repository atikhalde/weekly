...
def build_snapshot(symbol: str, security_id: str, exchange_segment: str,
    ...
    #
    # Two shifts stack up while the current week is still developing:
    #   1. request.security with lookahead_off refuses to reveal the in-progress
    #      weekly bar, so it reads from the last CONFIRMED week (here 20-Jul).
    #   2. the explicit [1] then steps one further back from that bar.
    # The window therefore ENDS TWO weeks before the developing one.
    #
    # `closed` already excludes the developing week, so its last row is the
    # confirmed bar. Applying [1] means dropping that row as well.
    #
    # Verified against the user's chart tables for 27-Jul:
    #   TMB    ours 847.00 -> chart 821.20   (excludes w/c 20-Jul)
    #   SENCO  ours 399.80 -> chart 390.40   (excludes w/c 20-Jul)
    # MONARCH/RATNAVEER/PYRAMID/CREDITACC are unchanged either way, because
    # their 26w maximum is older than 20-Jul.
    entry_level = float(highs[-(cfg.len_short + 1):-1].max())
    level_52 = float(highs[-(cfg.len_long + 1):-1].max())
    hi_short2 = float(highs[-(cfg.len_short + 2):-2].max())
...
def replay_week(snap: WeeklySnapshot, cfg: Strategy, bars: pd.DataFrame,
    ...
    for _, bar in bars.iterrows():
        ...
        bar_rvol = float("nan")
        if entry and getattr(cfg, "rvol_mode", "off") != "off":
            bar_rvol = compute_rvol(bars, history if history is not None else bars,
                                    ts, getattr(cfg, "rvol_lookback_sessions", 20))
            if cfg.rvol_mode == "on" and not np.isnan(bar_rvol) \

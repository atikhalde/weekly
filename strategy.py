...
def evaluate_bar(snap: WeeklySnapshot, cfg: Strategy, price: float,
    ...
    # c12 - market cap filter.
    #
    # UNKNOWN IS NOT SMALL. If the table has no row for a symbol (a new
    # listing, a renamed ticker, a failed fetch) we PASS it rather than block
    # it. c12 is mandatory inside gate_ok(), so treating "no data" as failure
    # would silently delete every such stock from the scan - the same class of
    # bug as the stale-level incident, but harder to notice because the alert
    # simply never arrives.
    #
    # The margin absorbs the few-percent error in shares-outstanding data, so
    # a borderline name stays in rather than being dropped on a rounding
    # difference against TradingView.
    if cfg.use_mcap:
        if snap.mcap is None:
            c12 = True
        else:
            margin = 1.0 - float(getattr(cfg, "mcap_margin_pct", 0.0) or 0.0) / 100.0
            c12 = snap.mcap > cfg.min_mcap * margin
    else:
        c12 = True
...
def gate_ok(snap: WeeklySnapshot, cfg: Strategy, ev: BarEval) -> bool:
    ...
    if cfg.gate_source == "live":
        ...
        # c12 (market cap) is always mandatory - it is a universe filter, not a
        # momentum opinion. c03 and c11 are mandatory by default, matching Pine,
        # but a multi-day model can switch them off: both measured NEGATIVE at a
        # 5-day horizon (c03 +2.80% when it passes vs +4.58% when it fails).
        mandatory = ["c12"]
        if getattr(cfg, "require_c03", True):
            mandatory.append("c03")
        if getattr(cfg, "require_c11", True):
            mandatory.append("c11")
        if not all(ev.conditions[k] for k in mandatory):
            return False
        # A row that is no longer required must not be counted as a failure
        # against gate_tolerance either.
        keys = tuple(k for k in keys
                     if not (k == "c03" and not getattr(cfg, "require_c03", True))
                     and not (k == "c11" and not getattr(cfg, "require_c11", True)))
...
def replay_week(snap: WeeklySnapshot, cfg: Strategy, bars: pd.DataFrame,
    ...
    #                       This is Pine's entryLevel and what Models A/B/C use.
    # "hi_short2"           highest(high, 26) as of TWO weeks ago - the level
    #                       printed in row 3 of the indicator table.
    #
    # hi_short2 is <= entry_level by construction, so it triggers EARLIER and
    # more often. When the most recent closed week made a new 26-week high the
    # two are identical and the models coincide.
    #
    # Chosen after the user observed CENTENKA: crossing the row-3 level (577.60)
    # caught a +13.0% MFE run with only -2.1% MAE, while the row-2 level (622)
    # never triggered at all that week.
    #
    # HONESTY NOTE: a point-in-time backtest of this trigger over 12 months
    # measured -0.05%/trade (t = -0.21, 95% CI [-0.48, +0.40]) - i.e. no
    # measurable edge. It ships as a PAPER model so live results decide.
    trigger = str(getattr(cfg, "trigger_level", "entry") or "entry").lower()
    level = snap.hi_short2 if trigger == "hi_short2" else snap.entry_level
    ...
    for _, bar in bars.iterrows():
        ...
        # Weekly volume conviction. Unlike c09 (a bare "above average" test)
        # this demands a MULTIPLE of the 10-week average, which is what
        # separated the 5-day winners: 2x -> +5.14%/trade vs +3.86% with no
        # requirement. Uses the same accumulating weekly volume c09 sees, so
        # early in the week it is naturally hard to satisfy.
        min_wvx = float(getattr(cfg, "min_weekly_vol_x", 0.0) or 0.0)
        if entry and min_wvx > 0.0:
            vs = ev.values.get("vol_sma", 0.0)
            if vs and vs > 0 and (week_volume / vs) < min_wvx:
                entry = False

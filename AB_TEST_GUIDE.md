...
> **UPDATE 29-Jul-2026 — Models A and B retired.**
>
> The intraday question is settled. Over 12 weeks and 449 real signals, every
> intraday exit rule tested was negative after a realistic 0.22% round trip
> (best −0.08%/trade, t = −3.3). A and B were both intraday, so both were
> removed; **only Model C (5-day swing) still runs.**
>
> The measurements below are kept because they are the evidence for that
> decision — B genuinely beat A (+0.63%/trade), but *both* lost money once
> costs were charged. Beating a losing model is not the same as winning.
>
> Current config lives in `models.yaml`. Reports now show a single column and
> state outright whether C is profitable rather than ranking anything.

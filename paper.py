...
def simulate(sig, bars_after: pd.DataFrame, capital: float,
    ...
    `fill`        "next-open"  entry at the next bar's OPEN (realistic; the
                               signal bar's close is only known once it ends)
                  "signal-close" entry at the signal bar's close (optimistic,
                               look-ahead; kept for comparison)
...
def main() -> int:
    ...
    ap.add_argument("--fill", choices=["next-open", "signal-close"],
                    default="next-open",
                    help="next-open (default, realistic): enter at the bar AFTER "
                         "the signal; signal-close: optimistic, uses the signal "
                         "bar's own close")

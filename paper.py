...
def simulate(sig, bars_after: pd.DataFrame, capital: float,
             bars_before: pd.DataFrame | None = None) -> PaperTrade:
    """
    Walk every 5m candle after entry and apply the two exit rules.

    `bars_after`  ascending 5m candles strictly AFTER the entry bar.
    `bars_before` the 5m candles up to AND INCLUDING the entry bar. Used to
                  warm the 9-EMA from real price history. Without it the EMA
                  would start flat at the entry price, and the very first close
                  below entry would sit under it - which exited every trade on
                  bar 1.
    """
    ...
    # 9-EMA of 5m closes, warmed on REAL candles up to and including the entry
    # bar - exactly what the EMA would read on a live chart at that moment.
    ...
    if ema_prev is None:
        # Not enough prior history (e.g. a Monday-open entry). Fall back to the
        # entry price, but suppress the EMA rule until enough candles have
        # accumulated, so we never fire a bogus bar-1 exit.
        ema_prev = entry
        warmup = EMA_LEN
    else:
        warmup = 0
            ...
            if snap is not None:
                ...
                if not wbars.empty:
                    ...
                    for sig in res.signals:
                        ...
                        trades.append(simulate(sig, after, args.capital, before))
                        found += 1

...
def load_models(path: Path | str | None = None) -> tuple[dict, list[Model]]:
    ...
    models = [Model(key=k, label=v.get("label", k),
...
#  Ledger
# --------------------------------------------------------------------------- #
LEDGER_COLS = ["model", "model_label", "symbol", "week", "signal_date",
...
def print_report(ledger: pd.DataFrame, models: list[Model]) -> None:
    print("\n" + "=" * 78)
    print("A/B PAPER TRADING — HEAD TO HEAD")
    print("=" * 78)
    ...
    if len(live) == 2:
        ...
        na, nb = sa["n"], sb["n"]
        if min(na, nb) < 10:
            print(f"VERDICT: too early. {ka} n={na}, {kb} n={nb}. "
                  "Need >= 10 closed trades each before the numbers mean much.")
        else:
            lead = ka if sa["avg_pct"] > sb["avg_pct"] else kb
            gap = abs(sa["avg_pct"] - sb["avg_pct"])
            print(f"VERDICT: {lead} leads by {gap:.2f}% per trade "
                  f"({ka} {sa['avg_pct']:+.2f}% vs {kb} {sb['avg_pct']:+.2f}%).")
            # crude significance
            print("         Treat as indicative until each side has ~30 trades.")
    else:
        print("VERDICT: need both models to produce trades.")
    print("-" * 78)
...
def telegram_summary(ledger: pd.DataFrame, models: list[Model]) -> str:
    ...
    if ledger.empty:
        return "🅰️🅱️ <b>A/B Paper Trading</b>\n\nNo trades recorded yet."
    days = sorted(ledger["signal_date"].astype(str).unique())
    stats = {m.key: summarise(ledger[ledger["model"] == m.key]) for m in models}

    lines = ["🅰️🅱️ <b>A/B Paper Trading</b>",
    ...
    if len(live) == 2:
        (ka, sa), (kb, sb) = live
        if min(sa["n"], sb["n"]) < 10:
            lines.append(f"⏳ Too early — need ≥10 trades each "
                         f"({ka} {sa['n']}, {kb} {sb['n']}).")
        else:
            lead = ka if sa["avg_pct"] > sb["avg_pct"] else kb
            gap = abs(sa["avg_pct"] - sb["avg_pct"])
            lines.append(f"🏁 <b>{lead}</b> leads by {gap:.2f}%/trade")
    return "\n".join(lines)
...
def _send_telegram(msg: str) -> None:
    try:
        ...
        c = _lc(None)
        tg = Telegram(c.secrets.telegram_bot_token, c.secrets.telegram_chat_id)
        tg.send(msg)
        print("\nTelegram: sent")
    except Exception as exc:                            # noqa: BLE001

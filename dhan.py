...
class DhanClient:
    ...
    def daily_candles(self, security_id: str, exchange_segment: str,
                      ...
                      chunk_days: int = 0) -> pd.DataFrame:
        """
        Daily candles for the whole window.

        Dhan's DAILY endpoint has no 90-day cap (that limit applies to
        intraday), so the default is ONE request covering the full range.
        Chunking multiplied the call count 6x and blew the rate limit.

        Set chunk_days > 0 to split the range if a very long window ever comes
        back truncated. Per-chunk failures are tolerated: a stock listed in
        2024 legitimately has no data for a 2021 chunk, and losing that chunk
        must not discard the years that DO exist.
        """
        ...
        for start, end in windows:
            ...
            try:
                part = self._to_frame(
                    self._post("/charts/historical", payload, self._data_limiter))

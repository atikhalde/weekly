...
def test_signal_survives_the_full_alert_pipeline(setup, tmp_path):
    ...
    # The header reports the REAL count. c01 (52W high) is not part of the
    # entry gate when req52 is off, so a valid signal may legitimately be
    # 12/13 - the message must state whichever is true, never hardcode 13/13.
    assert f"{sig.evaluation.pass_count}/13" in msg
    if sig.evaluation.all_ok:
        assert "PASS" in msg

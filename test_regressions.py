...
#  The 26W/52W breakout LEVELS were one week too recent. Pine reads them with
#      request.security(..., "W", ta.highest(high, N)[1], lookahead_off)
#  Two shifts stack while the week is developing: lookahead_off reads the last
#  CONFIRMED weekly bar, then [1] steps one further back. So the window ends
#  TWO weeks before the developing one, not one.
#
#  Measured against the user's 27-Jul chart tables:
#      TMB    ours 847.00 -> chart 821.20
#      SENCO  ours 399.80 -> chart 390.40
#  With 847.00 TMB could not cross until 14:15 and was dropped entirely; with
#  the correct 821.20 it breaks out on the morning candle, as the chart shows.
# --------------------------------------------------------------------------- #
def test_breakout_level_steps_back_one_extra_week():
    import numpy as np
    import pandas as pd
    ...
    correct = closed[-(cfg.len_short + 1):-1].max()
    naive = closed[-cfg.len_short:].max()

    assert snap.entry_level == pytest.approx(correct)
    assert snap.entry_level <= naive, (
        "the level must never include the most recent confirmed week - that "
        "made TMB's level 847.00 instead of 821.20 and lost the signal")

    correct52 = closed[-(cfg.len_long + 1):-1].max()
    assert snap.level_52 == pytest.approx(correct52)


def test_hi_short2_keeps_its_relative_offset():
    """c03 (fresh breakout) compares against a window one step older again."""
    import pandas as pd
    ...
    closed = wk[wk["week_start"] < target]["high"].to_numpy(float)
    assert snap.hi_short2 == pytest.approx(closed[-(cfg.len_short + 2):-2].max())
    assert snap.hi_short2 <= snap.entry_level or True   # ordering is data-dependent


# --------------------------------------------------------------------------- #
#  BUG 21 - a "successful" snapshot rebuild that changed nothing.
#  After the breakout-level fix the user re-ran Weekly Snapshot. It reported
#  SUCCESS in 9m20s - but build_snapshot resumes any rows already present for
#  the current week, found all 2104, and re-saved the OLD values. TMB stayed at
#  847.00 instead of 821.20, so the scanner would have used stale levels.
# --------------------------------------------------------------------------- #
def test_snapshot_rows_carry_a_logic_version():
    body = (ROOT / "build_snapshot.py").read_text()
    assert "LOGIC_VERSION" in body
    assert 'row["logic_version"] = LOGIC_VERSION' in body, \
        "each row must record the logic that produced it"


def test_resume_discards_rows_from_older_logic():
    """A snapshot built by different strategy code must not be reused."""
    import pandas as pd
    ...
    def resumable(df):
        prev = df[df["week_start"] == week]
        if "logic_version" in prev.columns:
            return prev[prev["logic_version"] == str(LOGIC_VERSION)]
        return prev.iloc[0:0]

    legacy = pd.DataFrame([{"symbol": "TMB", "week_start": week,
                            "entry_level": "847.00"}])
    assert len(resumable(legacy)) == 0, "pre-versioning rows must be rebuilt"

    older = pd.DataFrame([{"symbol": "TMB", "week_start": week,
                           "entry_level": "847.00", "logic_version": "1"}])
    assert len(resumable(older)) == 0, "older logic must be rebuilt"

    current = pd.DataFrame([{"symbol": "TMB", "week_start": week,
                             "entry_level": "821.20",
                             "logic_version": str(LOGIC_VERSION)}])
    assert len(resumable(current)) == 1, "matching logic should still resume"


def test_snapshot_workflow_exposes_fresh_flag():
    """--fresh existed in the script but the workflow had no way to pass it."""
    wf = load_workflow("snapshot.yml")
    trigger = wf.get("on") or wf.get(True)
    inputs = trigger["workflow_dispatch"]["inputs"]
    assert "fresh" in inputs, "operators need a way to force a full rebuild"
    body = (WORKFLOWS / "snapshot.yml").read_text()
    assert "'--fresh'" in body


# --------------------------------------------------------------------------- #
#  Relative volume (RVOL) - an optional, better-behaved volume test.
#  Pine's c09 compares an ACCUMULATING weekly total against a full-week
#  average, so it is near-impossible on Monday and near-free on Friday. RVOL
#  compares volume today up to this minute against the median volume up to the
#  same minute over prior sessions - equally meaningful at any hour.
#
#  Measured on the 27-Jul breakouts: valid median 84.5 vs false 4.1 (~20x),
#  where c09 separated them by ~1x. BUT with the full 13/13 gate already
#  applied it adds nothing (every survivor already has rvol > 11), and using it
#  to RELAX c09 always lowered precision. So it ships OFF.
# --------------------------------------------------------------------------- #
def test_rvol_defaults_to_off():
    from config import Strategy

    cfg = Strategy()
    assert cfg.rvol_mode == "off", (
        "measured on 27-Jul: strict Pine 67% precision vs 57% with "
        "vol=day+rvol>=15 - relaxing c09 never helped")


def test_rvol_measures_relative_participation():
    from datetime import datetime, timedelta
    from zoneinfo import ZoneInfo

    import pandas as pd

    from strategy import compute_rvol

    ist = ZoneInfo("Asia/Kolkata")

    def session(day, vol):
        base = datetime(2026, 7, day, 9, 15, tzinfo=ist)
        return [{"datetime": base + timedelta(minutes=5 * i), "volume": vol,
                 "open": 1, "high": 1, "low": 1, "close": 1} for i in range(12)]

    hist = pd.DataFrame(sum((session(d, 100) for d in (20, 21, 22, 23)), []))
    today = pd.DataFrame(session(27, 500))          # 5x the usual pace
    at = pd.Timestamp(today.iloc[5]["datetime"])

    rv = compute_rvol(today, pd.concat([hist, today], ignore_index=True), at)
    assert 4.5 < rv < 5.5, f"expected ~5x, got {rv}"


def test_rvol_returns_nan_when_unknown_not_zero():
    """
    Some feeds report 0 volume on the 09:15 opening bar. That is missing data,
    not weak participation - it must never be treated as a failed filter.
    """
    from datetime import datetime
    from zoneinfo import ZoneInfo
    ...
    ist = ZoneInfo("Asia/Kolkata")
    bars = pd.DataFrame([{"datetime": datetime(2026, 7, 27, 9, 15, tzinfo=ist),
                          "volume": 0, "open": 1, "high": 1, "low": 1, "close": 1}])
    assert np.isnan(compute_rvol(bars, bars, bars.iloc[0]["datetime"]))


def test_rvol_nan_never_blocks_an_entry():
    body = (ROOT / "strategy.py").read_text()
    block = body[body.index("if entry and getattr(cfg"):body.index("if entry:\n            took")]
    assert "not np.isnan(bar_rvol)" in block, \
        "an unknown rvol must not veto a signal"

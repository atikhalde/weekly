...
# --------------------------------------------------------------------------- #
def test_volume_condition_is_prorated_by_week_elapsed():
    """Weekly volume accumulates; the target must scale with time elapsed."""
    from config import Strategy
    from strategy import evaluate_bar

    snap = _gate_snap()
    cfg = Strategy(volume_prorate=True)

    # 1% into the week with 1% of the average volume = on pace -> pass
    early = evaluate_bar(snap, cfg, price=101.0, week_open=99.0,
                         week_volume=0.02, day_open=100.0, week_fraction=0.01)
    assert early.conditions["c09"] is True, "on-pace early volume must pass"

    # same instant, almost no volume -> genuinely weak -> fail
    dead = evaluate_bar(snap, cfg, price=101.0, week_open=99.0,
                        week_volume=0.0001, day_open=100.0, week_fraction=0.01)
    assert dead.conditions["c09"] is False

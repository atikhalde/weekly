...
def test_bug77_every_picks_field_survives_a_blank_cell():
    """
    THE SECOND CRASH. BUG 76 fixed `tradeable`; the very next A/B run died on
    the IDENTICAL pattern one field over:

        ValueError: cannot convert float NaN to integer
        ab_paper.py   sig.btst_age = int(float(pick.get("age") or 0))

    NaN is truthy, so `or 0` never fires and int(float(nan)) raises. Fixing one
    instance of a pattern without grepping for its siblings turned one outage
    into two.
    """
    from ab_paper import _f

    nan = float("nan")
    assert _f({"age": nan}, "age", 0.0) == 0.0, "blank CSV cell = NaN"
    assert _f({}, "age", 0.0) == 0.0
    assert _f({"age": ""}, "age", 0.0) == 0.0
    assert _f({"age": None}, "age", 0.0) == 0.0
    assert _f({"age": "junk"}, "age", 0.0) == 0.0
    assert _f({"age": 43}, "age", 0.0) == 43.0
    assert _f({"age": "7"}, "age", 0.0) == 7.0
    # int() of the result must never raise
    for v in (nan, None, "", "junk", 43, "7"):
        int(_f({"age": v}, "age", 0.0))

    # no raising pattern may remain anywhere in the file
    # strip comments AND docstrings - the fix is described in prose, and the
    # description necessarily quotes the broken form.
    import ast
    import re

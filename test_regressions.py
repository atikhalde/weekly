...
# --------------------------------------------------------------------------- #
def test_ab_models_file_is_valid_and_has_two_models():
    import yaml as _yaml

    raw = _yaml.safe_load((ROOT / "models.yaml").read_text())
    models = raw["models"]
    assert len(models) >= 2, "an A/B test needs at least two models"
    assert "A_gated" in models and "B_mover" in models

    # the two theses must actually DIFFER on the disputed knob
    a = models["A_gated"]["strategy"]
    b = models["B_mover"]["strategy"]
    assert a.get("drop_c09") is not b.get("drop_c09"), (
        "the whole experiment is whether dropping c09 helps - the two models "
...
def test_watchlist_buckets_are_correct():
    """FIRED / ABOVE / NEAR / FAR must key off the frozen level."""
    from watchlist import build_message

    rows = [
        dict(symbol="AAA", ltp=110.0, level=100.0, pct=10.0, gap=-10.0, bucket="ABOVE"),
        dict(symbol="BBB", ltp=98.0, level=100.0, pct=-2.0, gap=2.0, bucket="NEAR"),
        dict(symbol="CCC", ltp=50.0, level=100.0, pct=-50.0, gap=50.0, bucket="FAR"),
        dict(symbol="DDD", ltp=120.0, level=100.0, pct=20.0, gap=-20.0, bucket="FIRED"),
    ]
    msg = build_message("2026-07-27", rows, {"universe": 4}, 3.0)
    assert "AAA" in msg and "BBB" in msg and "DDD" in msg
    assert "CCC" not in msg, "FAR names are noise and must not be listed"
    assert "week of 2026-07-27" in msg


def test_watchlist_message_survives_a_full_universe():
    """
    2000 qualifying names must not produce an unbounded message. Telegram
    splits at 4096 and the per-section caps keep each chunk readable.
    """
    from watchlist import MAX_ABOVE, MAX_NEAR, build_message

    rows = [dict(symbol=f"SYM{i:04d}", ltp=105.0, level=100.0,
                 pct=5.0, gap=-5.0, bucket="ABOVE") for i in range(2000)]
    msg = build_message("2026-07-27", rows, {"universe": 2000}, 3.0)
    listed = msg.count("• <b>SYM")
    assert listed <= MAX_ABOVE, f"listed {listed} rows, cap is {MAX_ABOVE}"
    assert "see the CSV" in msg, "the truncation must point at the full file"

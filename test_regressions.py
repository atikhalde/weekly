...
def test_single_model_report_states_profitability(capsys):
    """With one model there is nothing to rank - say if it is winning."""
    import pandas as pd
...
def test_only_the_swing_model_remains():
    from ab_paper import load_models

    _defaults, models = load_models()
    assert [m.key for m in models] == ["C_swing"]
    assert models[0].is_swing
    assert models[0].hold_days == 5
    assert models[0].exit["stop_pct"] == 7.0
    assert models[0].exit["target_r"] == 3.0

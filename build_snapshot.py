...
from strategy import LOGIC_VERSION, build_snapshot, week_start_of

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)-7s %(message)s")
log = logging.getLogger("build_snapshot")

# LOGIC_VERSION now lives in strategy.py so writers and readers cannot drift.


# Market caps for Pine c12. Loaded once at import of main(); an empty table
# means every stock reports "unknown", which PASSES c12 rather than blocking
# the entire scan. Rebuild it with `python mcap.py`.
MCAP: dict[str, float] = {}
...
def main() -> int:
    ...
    global MCAP
    if cfg.strategy.use_mcap:
        MCAP = load_mcap_table(cfg.paths["mcap"])
        if MCAP:
            log.info("market-cap table: %d symbols (min %.0f Cr, margin %.1f%%)",
                     len(MCAP), cfg.strategy.min_mcap, cfg.strategy.mcap_margin_pct)
        else:
            log.warning("use_mcap is ON but %s is missing or empty - every "
                        "symbol will report UNKNOWN and pass c12. "
                        "Run `python mcap.py` to build it.", cfg.paths["mcap"])
    ...
    def work(ins: Instrument):
        ...
        snap = build_snapshot(ins.symbol, ins.security_id, ins.exchange_segment,
                              ...
                              mcap=MCAP.get(ins.symbol.upper()))
        if snap is None:

...
from dhan import IST, DhanClient, DhanError, Instrument, NoDataError, last_n_years
from strategy import build_snapshot, week_start_of

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)-7s %(message)s")
log = logging.getLogger("build_snapshot")

# Bump whenever anything that changes a stored value is edited (levels,
# indicator maths, snapshot columns). Rows built by an older version are
# discarded on resume instead of being silently reused.
#
# 3 (28-Jul-2026): breakout level settled as Option B -
#       entry_level = highest(high, 26) INCLUDING the last closed week.
#     Version 2 rows were written while the REVERTED one-week shift was live,
#     so they hold levels ~1% too low (TMB 821.20 instead of 847.00, RADICO
#     4161.80 instead of 4193.00). Those rows are indistinguishable from good
#     ones by content alone - RADICO's entry_level even equals its hi_short2 -
#     so the only safe move is to invalidate the whole generation.
#
#     LESSON: the version must be bumped when the formula is REVERTED too, not
#     just when it is changed. v2 was stamped in the same session as the bad
#     formula, so the guard could not see its own bad output.
LOGIC_VERSION = 3
...
def main() -> int:
    ...
    ap.add_argument("--fresh", action="store_true",
                    help="ignore any existing snapshot and rebuild from scratch")
    ...
    # ---- RESUME ------------------------------------------------------------
    # A full universe build takes many minutes. If a run is cancelled or times
    # out, everything done so far used to be thrown away. Keep any rows already
    # saved for THIS week and skip those symbols.
    snap_path = cfg.paths["snapshot"]
    done_symbols: set[str] = set()
    if not args.fresh and snap_path.exists():
        try:
            prev = pd.read_csv(snap_path, dtype=str)
            prev = prev[prev["week_start"] == str(target_week.date())]
            # Only resume rows built by the SAME strategy logic. Without this a
            # rerun after a level/indicator change silently keeps the old
            # numbers and reports success in a few minutes - the run looks
            # healthy while the file still holds the previous formula's levels.
            if "logic_version" in prev.columns:
                prev = prev[prev["logic_version"] == str(LOGIC_VERSION)]
            else:
                prev = prev.iloc[0:0]
                log.warning("existing snapshot predates logic versioning - "
                            "rebuilding from scratch")
            if not prev.empty:
                rows = prev.to_dict("records")
                done_symbols = {str(r["symbol"]).upper() for r in rows}
                log.info("resuming: %d symbols already done for week %s",
                         len(done_symbols), target_week.date())
        except (OSError, KeyError, ValueError) as exc:
            log.warning("could not read existing snapshot (%s) - starting fresh", exc)

    pending = [i for i in instruments if i.symbol.upper() not in done_symbols]
    if not pending:
        log.info("nothing to do - snapshot already complete for this week")
        return 0
    if done_symbols:
        log.info("%d symbols remaining", len(pending))

    def flush() -> None:
        """Write what we have so far, atomically."""
        if not rows:
            return
        tmp = snap_path.with_suffix(".tmp")
        pd.DataFrame(rows).sort_values("symbol").to_csv(tmp, index=False)
    ...
    def work(ins: Instrument):
        ...
        if snap is None:
            first = daily.iloc[0]["datetime"].date() if len(daily) else "?"
            last = daily.iloc[-1]["datetime"].date() if len(daily) else "?"
            from strategy import build_weekly_bars
            wk = build_weekly_bars(daily)
            closed = wk[wk["week_start"] < target_week]
            return ins, None, (f"short_history: {len(daily)} daily bars "
                               f"{first}..{last} -> {len(closed)} closed weeks "
                               f"(need ~53)")
        ...
        for n, fut in enumerate(as_completed(futures), 1):
            ...
            try:
                ...
                if snap is None:
                    ...
                    row = snap.to_row()
                    row["logic_version"] = LOGIC_VERSION
                    rows.append(row)
            except NoDataError:
                skipped += 1
                no_data += 1
            ...
            if n % 200 == 0:
                flush()          # checkpoint, so a cancel keeps the work
                log.info("  %d/%d  ok=%d skip=%d (nodata=%d short=%d) err=%d  %.0fs",
                         n, len(pending), len(rows), skipped, no_data, short_hist,

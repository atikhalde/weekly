...
def main() -> int:
    ...
    with ThreadPoolExecutor(max_workers=cfg.runtime.max_workers) as pool:
        ...
        for n, fut in enumerate(as_completed(futures), 1):
            ...
            if n % 200 == 0:
                log.info("  %d/%d  ok=%d skip=%d err=%d  %.0fs",
                         n, len(instruments), len(rows), skipped, errors, time.time() - started)

    if not rows:
        log.error("no snapshots produced from %d symbols (skipped=%d, errors=%d)",
                  len(instruments), skipped, errors)
        if errors > skipped:
    ...
    log.info("wrote %d snapshots (skipped %d, errors %d) in %.0fs -> %s",
             len(df), skipped, errors, time.time() - started, cfg.paths["snapshot"])
    return 0

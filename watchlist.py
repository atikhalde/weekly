...
# Everything else (c01/c02 price, c09 accumulating volume, c10 week-green,
# c13 day-green) needs the developing week and would be meaningless at 08:45,
# so scoring them would just add noise that changes every morning.
#
# c04/c06/c07/c08 use the g_* CLOSED-WEEK values the snapshot already carries -
# the same proxies gate_ok() falls back on - so the score is stable all week.
...
def score_conditions(s: WeeklySnapshot, cfg, mcap: float | None = None,
    ...
    """
    How many of the pre-market-knowable rows this name already satisfies.
...
def build_message(week: str, rows: list[dict], counts: dict,
    ...
    # Rank by conditions passed (desc), then distance to the level (asc).
    # Distance alone put a 4/8 name that happened to be 0.1% away above an 8/8
    # name 0.4% away; the 8/8 is the better setup even a shade further out.
    watch = sorted((r for r in rows if r["bucket"] == "WATCH"),
                   key=lambda x: (-x.get("score", 0), x["gap"]))
    ...
    if watch:
        ...
        lines += ["", f"👀 <b>CLOSEST TO BREAKOUT (top {len(shown)})</b>",
                  f"<i>within {near_pct:g}% of the level · ranked by "
                  f"conditions passed, then distance</i>"]
        for r in shown:
            tag = r.get("which") or ""
            cap = r.get("mcap_cr")
            sc = r.get("score")
            mx = r.get("max_score", len(SCORED_ROWS))
            head = f"<b>{sc}/{mx}</b> " if sc is not None else ""

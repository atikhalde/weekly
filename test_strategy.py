...
def test_snapshot_excludes_the_target_week(daily, cfg):
    """
    The frozen level must come from CLOSED weeks only - no look-ahead - and
    the window ENDS AT and INCLUDES the last closed week, matching Pine's

        request.security(..., "W", ta.highest(high, N)[1], lookahead_off)

    where [1] steps back one bar from the DEVELOPING week. Verified live on
    TMB (Tue 28-Jul 15:08 mid-session): 26W level 847.00 = high of w/c 20-Jul,
    the last closed week, so that week is inside the window.
    """
    wk = build_weekly_bars(daily)
    ...
    # window ends AT the last closed week and includes it
    expected = closed["high"].tail(cfg.len_short).max()

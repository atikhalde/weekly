...
def format_signal(sig: Signal) -> str:
    ...
    lines = [
        ...
        f"(+{pct:.2f}%)",
        f"🕐 <b>Entry {sig.bar_time.strftime('%H:%M')} IST</b> "
        f"({sig.bar_time.strftime('%d-%b-%Y')})",
        f"    5m candle {sig.bar_time.strftime('%H:%M')}"
        f"-{(sig.bar_time + timedelta(minutes=5)).strftime('%H:%M')} · "
        f"entry <b>{_fmt(sig.price)}</b>",
        "",
...
def _compact(sig: Signal) -> str:
    ...
    return (f"🟢 <b>{_esc(sig.symbol)}</b>{tag} — <b>{_fmt(sig.price)}</b> "
            ...
            f"   entry <b>{sig.bar_time.strftime('%H:%M')} IST</b> · "
